...
1. **替换源图片**：把你的 PNG 图标替换 `assets/whale.png`（任意尺寸，会自动缩放到 256×256），并删除build目录下的.ico文件
...
启动器会捕获 dsh web 的所有输出。正常启动时 dsh web 仅输出一行本地网址（如 `dsh web: http://127.0.0.1:3080`），此时不会打扰用户；一旦 dsh web 输出了**除该网址以外的内容**（错误、警告、依赖缺失、端口冲突等），启动器会立即弹窗显示这些异常输出，方便及时排查问题。该机制对首次启动、托盘「打开」和「重启」均生效。
